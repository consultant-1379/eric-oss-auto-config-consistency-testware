...
DOCKER_CONTAINER_NAME="EACC_rApp_preonboarding_cleanup_$(date +%d-%m-%y_%H-%M-%S)_${BUILD_NUMBER}"

COMMAND="docker run --init --rm \
       --name ${DOCKER_CONTAINER_NAME} \
       --volume ${KUBECONFIG}:/.kube/config \
       --env KUBECONFIG=/.kube/config \
       ${KUBE_DOCKER_IMAGE} kubectl get pvc -n ${NAMESPACE} -l app=eric-oss-auto-config-consistency-pg
"
